...
{
	Edge	e = {sqs[0]->did, sqs[1]->did, dist};
	if (e.dist < max_dist) {
	    if (e.u > e.v) std::swap(e.u, e.v);
	    vedges.push_back(e);
	}
...
AdjacentMat::AdjacentMat(const char* fn, FTYPE distthr)
	  ...
	  n_vertex(0), n_edge(0), mname(0), verdid(0), edges(0)
{
	...
	n_edge = n_vertex = 0;
	while (fgets(str, MAXL, fd)) ++n_edge;  // number of edges
	rewind(fd);
	int	maxver = 2 * n_edge;
	...
	KVpair<INT, int>* kv = verhash.begin();
	KVpair<INT, int>* kz = verhash.end();
	n_vertex = 0;
	for ( ; kv < kz; ++kv)
	    if (kv->val) ++n_vertex;    // count
	mname = new const char*[n_vertex];
	n_vertex = 0;
	kv = verhash.begin();
	for ( ; kv < kz; ++kv) {
	    if (kv->val) {	      // str to number
		mname[n_vertex] = verhash.memname(kv->key);
		kv->val = ++n_vertex;   // start from 1
	    }
	}
	rewind(fd);
	...
	while (fgets(str, MAXL, fd)) {
	    ...
	    int q = 1 - p;
	    kv = verhash.find(mu[p]);
	    e->u = kv->val - 1;
	    kv = verhash.find(mu[q]);
	    e->v = kv->val - 1;
	    if (e->u > e->v) std::swap(e->u, e->v);
