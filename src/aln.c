...
template <class seq_t>
VTYPE alnoutput(CalcServer<seq_t>* svr, seq_t** sqs, PwdM* pwd, Gsinfo* GsI, ThQueue<seq_t>* q)
{
	seq_t*&	a = sqs[0];
	seq_t*&	b = sqs[1];
	if (pwd->swp) {
	...
	if (pwd->DvsP == 1 || pwd->DvsP == 2) {	// DNA vs protein
		...
		if (getlpw()) {		/* print alignment */
		    ...
		    print2((Seq**) sqs, gaps, (double) GsI->scr, GsI, 1, 1, print2Skip);
		} else
		    repalninf((Seq**) sqs, GsI);
	    } else if (gene) {
		printgene((Seq**) sqs, GsI);
	    }
...
void swgscore(CalcServer<seq_t>* svr, seq_t** sqs, VTYPE val, ThQueue<seq_t>* q = 0)
{
	m_thread_Lock(q);
	svr->fpavsd(val);
	fprintf(out_fd, "%6.2lf ", (double) val);
	fprint_seq_mem((Seq**) sqs, 2);
	m_thread_Unlock(q);
}
...
static	const	char smark[] = ">-^<";
	...
	if ((algmode.lcl & 16) && !algmode.qck && !algmode.lsg) {
		...
		scr = (swg2nd(sqs, pwd, &AlnI, clns->at()) &&
		   (!algmode.thr || AlnI.fstat.val >= pwd->Vthr))?
...
match2end:
	    ...
	    scr = (dir != ERROR && (!algmode.thr || GsI[dir].fstat.val >= pwd->Vthr))?
...
void aln_setup(CalcServer<mSeq>* svr)
{
	...
	initSsHpPrm();
#endif
	int	dvsp = prePwd((Seq**) svr->in_face);
	if (algmode.qck || dvsp == 1 || dvsp == 2 || a->inex.intr || b->inex.intr)
	    makeWlprms(dvsp);
...
static PwdM* aln_reset(PwdM* pwd, mSeq** sqs)
{
	...
	if (!pwd) {
	    prePwd((Seq**) sqs);
	    pwd = new PwdM(sqs);
	} else if (dvsp != pwd->DvsP || (a->many > 1 || b->many > 1)) {
	    int	swp = pwd->swp;
	    pwd = new PwdM(sqs);
	    if (dvsp == 1 || dvsp == 2) pwd->swp = swp;
	} else {
	    pwd->reset_ab(sqs);
	    pwd->selAlnMode(sqs, false);
	}
	return pwd;
}
...
static void genomicseq(Seq** seqs, PwdB* pwd, bool reverse = false)
{
...
int aln_main(CalcServer<mSeq>* svr, mSeq** sqs, ThQueue<mSeq>* q = 0)
{
	...
	if (algmode.lsg) genomicseq((Seq**) (sqs + 1), pwd, true);
...
MakeMsa<mSeq>::MakeMsa(AlnServer<mSeq>* sv): svr(sv)
{
	...
	if (ins_a->getseq(svr->memb->name[i++])) fatal("Nothing to do !\n");
	while (ins_b->getseq(svr->memb->name[i++])) {
	...
	if (ss && rename(ss, *OutPrm.out_file? OutPrm.out_file: Def_out_file))
	    perror("Error renaming file ");
}
