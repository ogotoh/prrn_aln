...
protected:
	...
	VTYPE	spjthr;
public:
	Aln2h1(mSeq** _seqs, PwdM* _pwd, WINDOW* _wdw);
...
static	void	addsigEjxt(JUXT* jxt, int num, mSeq* b);

// initialization of alignment process

Aln2h1::Aln2h1(mSeq* _seqs[], PwdM* _pwd, WINDOW* _wdw) :
...
//	diagonal match
		...
		if (n > b->left + 2) {
			...
			*h = *sj;
			sj->dir = 0;
		    } else     h->val += (pwd->*pwd->Sim2)(as, bs - bbt) + sigE;
...
Acceptor:
		    if (phs == -1)
			sj->val =  mx->val + (pwd->*pwd->Sim2)(as + abt, bs + 2 * bbt);
		    int nb = n - phs;
			...
			for (int l = 0; l < INTR; ++l) {
				...
				if (isdiag(phl)) y += (pwd->*pwd->Sim2)(as, cs);
			    } else if (phs == -1) {
				CHAR*	cs = spjcs->spjseq(phl->jnc, nb) + bbt;
				y += pwd->pmt->prematT(cs);
				x = y + (pwd->*pwd->Sim2)(as + abt, cs);
				if (x > sj->val) {
				    maxdhl = phl;
				    sj->val = x;
				}
			...
			if (phs == 1) {
			    ...
			    VTYPE	x = y + (pwd->*pwd->Sim2)(as + abt, cs);
			    if (x > sj->val) {
				maxdhl = phl;
				sj->val = x;
			    }
...
VTYPE skl_rngH_ng(mSeq* seqs[], Gsinfo* gsi, PwdM* pwd)
{
	mSeq*	a = seqs[0];
	mSeq*	b = seqs[1];
	int	abt = a->byte;
	...
	FSTAT*	fst = &gsi->fstat;
	FSTAT	pst;
	Eijnc*	eijnc = new Eijnc();
static	const char*	fswarn = "%s %s FrameShift: %d %d\n";

	memset(fst, '\0', sizeof(FSTAT));	// clear fst
	memset(&pst, '\0', sizeof(FSTAT));	// clear pst
	memset(&rbuf, '\0', sizeof(EISCR));	// clear rbuf
	CHAR*	cs = b->cfat(wsk[num-1].n - 2);
	...
	if ((algmode.lcl & 1) && bb[1].sigS > h) h = bb[1].sigS;
	if ((algmode.lcl & 4) && bb->sig3 > h)	 h = bb->sig3;
	rbuf.left = n;
	rbuf.rleft = m;
	rbuf.iscr = NEVSEL;
	rbuf.sig3 = h;
	while (--num) {
	    ...
	    int	mi = (wsk->m - m) * 3;
	    if (mi && insert) {
		VTYPE x = SumCodePot(bbi, insert, 0, pwd)
	    ...
	    int	ni = wsk->n - n;
	    if (ni && deletn && mi) { // skip intron when mi == 0
	    ...
	    m += d / 3;
	    VTYPE	x = 0;
	    for ( ; d > 2; d -= 3, as += abt, bs += bbt3, bb += 3) {
		hvl = cs?
			(pwd->*pwd->Sim2)(as, cs + bbt) + pwd->pmt->prematT(cs + bbt):
			(pwd->*pwd->Sim2)(as, bs + bbt) + bb[1].sigE;
		x += hvl;
		if (a->inex.vect) {
		    VTYPE*  aprof = (VTYPE*) as;
		    if (a->inex.vect == VECPRO) aprof -= a->felm;
		    fst->mch += aprof[bs[bbt]];
		    fst->mmc += a->sumwt - aprof[bs[bbt]] 
			- aprof[nil_code] - aprof[gap_code];
		    fst->unp += aprof[nil_code] + aprof[gap_code];
		} else {
#if USE_WEIGHT
		    FTYPE*	wt = a->weight;
#endif
		    cs = as;
		    for (int j = 0; j < abt; ++j, ++cs) {
			if (*cs == bs[bbt] || (*cs == SER && bs[bbt] == SER2)) {
#if USE_WEIGHT
			    if (wt) fst->mch += *wt++;
			    else
#endif
		...
		if (isJunct(phs, phs5)) {
		    ...
		    if (sigii) xi += SpbFact * sigii[3 * m - phs];
		    if (phs != 0) cs = spjcs->spjseq(nb, n3);
		    if (insert == 0 && phs == 1)
			xi += (pwd->*pwd->Sim2)(as - abt, cs) - hvl + pwd->pmt->prematT(cs);
		}
		if (x > xi) {
		    xi = x;
		    phs = -1;
		...
		x = SumCodePot(bb, i, cs? cs + bbt: 0, pwd);
		if (xi > x + pwd->GapPenalty3(i) && xi > rbuf.iscr) {	// intron
		    if (xc2 > NEVSEL) xc = xc2;
		    xc2 = xc + x;
		    cs2 = cs;
		    intlen = i;
		    rbuf.right = nb;
		    ...
		    rbuf.phs = phs;
		    rbuf.iscr = xi;
		    rbuf.escr = h + xc + pwd->GapPenalty3(insert);
		    rbuf.sig5 = sig5;
		    ...
		    if (xc2 > NEVSEL && cs != cs2)
			x = SumCodePot(bb, i, cs2? cs2 + bbt: 0, pwd);
		    xc += x;
		    cs = 0;
		}
		bb = b3;
	...
	if (n >= 2) {
	    ...
	    if (bb + 3 < bb_last) {
		VTYPE	x = 0;
		if (algmode.lcl & 2 && bb[3].sigT > 0)
		    x = xc2 = bb[3].sigT;
		if (algmode.lcl & 8 && bb[2].sig5 > 0 && bb[2].sig5 > bb[3].sigT)
		    x = xc2 = bb[2].sig5;
		h += x;
	    }
	}
...
DonRev:
			...
			for (int l = 0; l < INTR; ++l) {
			    ...
			    if (phs == -1) {
				CHAR*	cs = spjcs->spjseq(nb, phl->jnc) + bbt;
				y += pwd->pmt->prematT(cs);
				if (isdiag(phl)) y += (pwd->*pwd->Sim2)(as, cs);
			    } else if (phs == 1) {
...
//	diagonal match
		...
		if (n > b->left + 2) {
			...
			*h = *sj;
		    ...
		    else h->val += (pwd->*pwd->Sim2)(as, bs - 2 * bbt) + sigE;
...
AccFwd:
		    if (phs == -1)
			sj->val = mx->val + (pwd->*pwd->Sim2)(as + abt, bs + bbt); 
		    int		nb = n - phs;
		    VTYPE	sigJ = sigii? SpbFact * sigii[3 * m - phs]: 0;
		    WRECD*	maxdhl = 0;
		    for (int d = 0; d < pwd->Noll; ++d) {
			...
			for (l = 0; l < INTR; ++l) {
				...
				CHAR*	cs = spjcs->spjseq(phl->jnc, nb);
				y += pwd->pmt->prematT(cs);
				if (phl->dir & DIAG) y += (pwd->*pwd->Sim2)(as, cs);
			    } else if (phs == -1) {
				CHAR*	cs = spjcs->spjseq(phl->jnc, nb) + bbt;
				y += pwd->pmt->prematT(cs);
				x = y + (pwd->*pwd->Sim2)(as + abt, cs);
...
FindCenter:
		if (m == mm) {
			...
			y = sj->val + (*hb)->val - (pwd->*pwd->Sim2)(as + abt, bs + bbt) - bb[1].sigE;
			if (y > x || (*hf)->dir & SPF2) {
			    x = y;
			    acc = SPJC;
			}
			...
			y = sjb->val + (*hf)->val - (pwd->*pwd->Sim2)(as, bs - 2 * bbt) - bb[-2].sigE;
			if (y > x || (*hb)->dir & SPF2) {
			    x = y;
			    dnr = SPJC;
			}
...
//	diagonal match
		...
		if (n < b->right - 2) {
		    *hq = *h;
		    h->val += (pwd->*pwd->Sim2)(as, bs + bbt) + sigE;
		    h->dir = isdiag(from)? DIAG: NEWD;
		} else	*h = ooml;
...
Donor:
			...
			for (l = 0; l < NCANDS; ++l) {
				...
				CHAR*	cs = spjcs->spjseq(nb, phr->jnc) + bbt;
				x += pwd->pmt->prematT(cs) +
					(pwd->*pwd->Sim2)(as + abt, cs);
			    }
...
{
	int	bbt3 = 3 * bbt;
	CHAR*	as = a->cfat(a->left);
	CHAR*	bs = b->cfat(b->left + 1);
	...
	for (int m = a->left; m < a->right; ) {
	    scr += (pwd->*pwd->Sim2)(as, bs) + bb->sigE;
	    as += abt;
	    bs += bbt3;
	    bb += 3;
	    ++m;
...
VTYPE Aln2h1::creepback(int ovr, VTYPE bscr, BOUND& lub)
{
	...
	while (a->left > lub.la && b->left > lub.lb
		&& (ovr < 0 || dscr < bscr)) {
	    as -= abt; bs -= bbt3; bb -= 3;
	    dscr += (pwd->*pwd->Sim2)(as, bs) + bb->sigE;
	    a->left--; b->left -= 3;
...
{
	CHAR*	as = a->cfat(a->right);
	CHAR*	bs = b->cfat(b->right + 1);
...
bool Aln2h1::indelfreespjH(int agap, VTYPE& iscr)
{
	...
	backward[--i] = 0;
	CHAR*	as = a->cfat(a->left - 1);	// cancel score of
	CHAR*	bs = b->cfat(b->left - 2);	// donor side overlap
	EXIN*	bd = b->exin->score(b->left - 2);
	for (VTYPE v = 0; --i >= 0; --as, bs -= 3, bd -= 3)
	    backward[i] = v += (pwd->*pwd->Sim2)(as, bs) + bd->sigE;
	int	phs = 1;
	int	m = a->right - 1;
	int	mm = m;
	int	n = d5;
	...
	for (VTYPE v = i = 0; n <= d3; ++n, ++bd, ++ba) {
		...
		VTYPE	x = (sigii? SpbFact * sigii[3 * m + phs]: 0)
			+ b->exin->sig53(n, n + ilen, IE5P3) + ip;
		VTYPE	y = x - v - backward[i];
		if (phs) {
		    CHAR*	cs = spjcs->spjseq(n, n + ilen);
		    if (phs == 1) cs++;
		    y += pwd->pmt->prematT(cs) + (pwd->*pwd->Sim2)(as, cs);
		}
	        if (y > iscr) {nn = n - phs; mm = m; iscr = y; maxspj = x;}
	    }
	    if (++phs == 0) ++i;
	    else if (phs == 2) {++m; phs = -1;}
	    else v += (pwd->*pwd->Sim2)(++as, bs += 3) + ba[1].sigE;
...
VTYPE Aln2h1::seededH_ng(INT level, int eimode, BOUND& lub)
{
	...
	RANGE	rng[2];
	int	cmode = eimode;
	bool	qck = algmode.blk && level == algmode.qck && algmode.lcl < 16;
	...
	if (num) {
		...
		else if (cmode == 3 && agap < 2 && bgap >= IntronPrm.llmt &&
...
//			scr += pincerTrcbkH_ng(cmode, iscore);
	    ...
	    bab.ua = lub.ua;
	    bab.ub = lub.ub;
	    if (eimode == 2 || (level == INT(algmode.lvl + 1) && eimode == 1))
		cmode = 2;
	}
	...
	if (cmode == 2 && agap == 0) {			// 3' end
	    if (wjxt) scr += cds3end(wjxt->jx, wjxt->jy - bgap);
	} else if (cmode == 3 && agap < 2 && bgap >= IntronPrm.llmt &&
		indelfreespjH(agap, iscore))
	    scr += iscore;
	else if (level < algmode.qck && ovr >= wlmt)
	    ...
	    scr -= creepfwrd(ovr, slmt, bab);
	    float	dpspace = (float) agap * (float) bgap / MEGA;
	    if (cmode == 2 && (qck || dpspace >= alprm.maxsp)) {
		if (wjxt) {
		    int	br = bgap - wjxt->jy - term;	// near 3' end
		    if (br < b->right) b->right = br;
		}
		pincerTrcbkH_ng(cmode, iscore);
		scr += iscore;
	    } else if (cmode == 3 && agap < elmt && bgap >= IntronPrm.llmt
...
//		scr += pincerTrcbkH_ng(cmode. iscore);
		if (wjxt) {	// define the alinged right ends
		    wjxt->jx = wjxt[-1].jx + wjxt[-1].jlen;
		    wjxt->jy = wjxt[-1].jy + 3 * wjxt[-1].jlen;
		    mfd->write((UPTR) wjxt);
		}
	    }
	}
	rest_range((Seq**) seqs, rng, 2);
	if (--level == algmode.lvl && wjxt) {
	    wjxt->jx = a->len;
	    wjxt->jy = b->len;
	}
...
SKL* Aln2h1::globalH_ng(VTYPE* scr)
{
	...
	if (algmode.qck) {
	    ...
	    *scr = seededH_ng(algmode.lvl, 1, bab);
	} else
	    *scr = lspH_ng();
	wsk.n = (int) mfd->size();
...
VTYPE HomScoreH_ng(mSeq* seqs[], PwdM* pwd)
{
	WINDOW	wdw;
...
Colonies* swg1stH_ng(mSeq* seqs[], PwdM* pwd, VTYPE* scr)
{
	WINDOW  wdw;
...
SKL* swg2ndH_ng(mSeq* seqs[], PwdM* pwd, VTYPE* scr, COLONY* clny)
{
...
SKL* alignH_ng(mSeq* seqs[], PwdM* pwd, VTYPE* scr)
{
	WINDOW  wdw;

	if (!algmode.blk && algmode.mns != 1) {
	    if (algmode.mns == 2) antiseq(seqs + 1);
	    else	geneorient((Seq**) seqs, pwd);
	}
	stripe31((Seq**) seqs, &wdw, alprm.sh); // Recacl. window boundaries
	Aln2h1 alnv(seqs, pwd, &wdw);
	return (alnv.globalH_ng(scr));
