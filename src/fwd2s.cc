...
template<>
void Fwd2s<RVPDJ_nv>::initializeS(int recsize)
{
	RVPDJ_nv*	r = recbuf;
...
template<>
VTYPE Fwd2s<RVPDJ_nv>::gapopen(const RVPDJ_nv* rcd, const mSeqItr& asi, int d3)
{
...
template<>
void Fwd2s<RVPDJ_nv>::update(RVPDJ_nv* dst, const RVPDJ_nv* src, const mSeqItr& asi, 
	VTYPE gop, int d3)
{
	if (d3 == 0) {
	    dst->gla = dst->glb = 0;
...
template <>
void Fwd2s<RVPDJ_nv>::update(RVPDJ_nv* dst, const mSeqItr& asi, int d3)
{
	if (d3 == 0) {
	    dst->gla = dst->glb = 0;
	} else if (d3 > 0) {
...
template<>
void Fwd2s<RVPDJ_hf>::initializeS(int recsize)
{
...
template<>
VTYPE Fwd2s<RVPDJ_hf>::gapopen(const RVPDJ_hf* rcd, const mSeqItr& asi, int d3)
{
	if (d3 == 0)
...
template<>
void Fwd2s<RVPDJ_hf>::update(RVPDJ_hf* dst, const RVPDJ_hf* src,
...
template<>
void Fwd2s<RVPDJ_hf>::update(RVPDJ_hf* rcd, const mSeqItr& asi, int d3)
{
	if (d3 == 0) {
