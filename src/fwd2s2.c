...
protected:
	...
	Exinon*	exin;
public:
	Aln2s1(mSeq** _seqs, PwdM* _pwd, WINDOW* _wdw);
	~Aln2s1() {delete mfd;}
...
extern	Colonies* swg1stS_ng(mSeq* seqs[], PwdM* pwd, VTYPE* scr);
extern	SKL*	swg2ndS_ng(mSeq* seqs[], PwdM* pwd, VTYPE* scr, COLONY* clny);
...
Aln2s1::Aln2s1(mSeq* _seqs[], PwdM* _pwd, WINDOW* _wdw) :
	seqs(_seqs), a(seqs[0]), b(seqs[1]), wdw(_wdw), pwd(_pwd),
	mfd(0), vmf(0), exin(b->exin)
{ }
...
VTYPE skl_rngS_ng(mSeq* seqs[], Gsinfo* gsi, PwdM* pwd)
{
	mSeq*	a = seqs[0];
	mSeq*	b = seqs[1];
	Exinon*	exin = b->exin;
	...
	FSTAT*	fst = &gsi->fstat;
	FSTAT	pst;
	FSTAT*	fstque = new FSTAT[alprm2.jneibr];
	...
	while (--num) {
		...
		if (xi >= x) {  /* intron */
		    hb = ha;
		    if (rbuf.right - rbuf.left > 0) eijnc->push(&rbuf);
	    ...
	    VTYPE	x = 0;
	    for ( ; d; --d, ++asi, ++bsi) {
		x += (pwd->*pwd->Sim2)(asi, bsi);
		CHAR*	cs = asi.res;
#if USE_WEIGHT
		FTYPE*	wt = a->weight;
#endif
		for (int j = 0; j < asi.many; ++j, ++cs) {
		    if (*cs == *bsi.res) {
#if USE_WEIGHT
			if (wt) fst->mch += *wt++;
			else
		...
		if (n++ - rbuf.left == alprm2.jneibr) {
		    rbuf.mch5 = (int) (fst->mch - pst.mch);
		    rbuf.mmc5 = (int) (fst->mmc - pst.mmc);
		    rbuf.unp5 = (int) (fst->unp - pst.unp);
		    rbuf.gap5 = (int) (fst->gap - pst.gap);
		}
		fstque[q] = *fst;
		...
		if (xi > pwd->GapPenalty(i) && xi > rbuf.iscr) {	/* intron */
		    intlen = i;
		    rbuf.right = n;
		    rbuf.rright = m;
		    rbuf.iscr = xi;
		    ...
		    rbuf.mch3 = (int) (fst->mch - fstque[q].mch);
		    rbuf.mmc3 = (int) (fst->mmc - fstque[q].mmc);
		    rbuf.unp3 = (int) (fst->unp - fstque[q].unp);
		    rbuf.gap3 = (int) (fst->gap - fstque[q].gap);
		    vclear(fstque, alprm2.jneibr);
		    q = 0;
		    pst = *fst;
...
/*	intron 3' boundary reverse phase	*/
	...
	if (cmode == 1) {
	    if (!a->inex.exgl && !b->inex.exgl)		// global
...
/*	intron 5' boundary	*/
		if (cmode == 3 && exin->isDonor(n)) {
			...
			for (l = 0; l < NCANDS; ++l) {
			    phr = hhl[m] + nnl[m][l];
			    if (!phr->dir) break;
			    x = phl->val + phr->val + pwd->IntPen->Penalty(phr->jnc - n)
				+ sigJ + exin->sig53(n, phr->jnc, IE35);
	...
	if (cmode == 2) {
	    if (!a->inex.exgr && !b->inex.exgr)		// global
...
VTYPE Aln2s1::diagonalS_ng()
{
	...
	for (int m = a->left; m++ < a->right; ++asi, ++bsi) {
...
bool Aln2s1::indelfreespjS(int agap, VTYPE& iscr)
{
	...
	mSeqItr	asi(a, a->left);	// cancel score of
	mSeqItr	bsi(b, b->left);	// donor side overlap
	for (VTYPE v = 0; --i >= 0; v += (pwd->*pwd->Sim2)(--asi, --bsi))
	    backward[i] = v;
	int	m = a->right;
	int	mm = m;
	int	n = d5;
	...
	bool	usespb = api.size() && use_spb();
	for (VTYPE v = i = 0; n <= d3; ++n, ++i, ++m, ++asi, ++bsi) {
	    ...
	    v += (pwd->*pwd->Sim2)(asi, bsi);	// cancel score of acc side
	    if (a_in_zone) ++api;
	}
	delete[]	backward;
	if (maxspj > NEVSEL) {
...
{
	mSeqItr	asi(a, a->right);
	mSeqItr	bsi(b, b->right);
	VTYPE	dscr = 0;
...
VTYPE Aln2s1::seededS_ng(INT level, int eimode, BOUND& lub)
{
	...
	if (num) {
		...
		if (cmode == 1 && agap == 0)	// 5' end
			...
			stripe((Seq**) seqs, wdw, alprm.sh);
			scr += pincerTrcbkS_ng(cmode);
		    } else if (cmode == 3 && agap < IntronPrm.elmt
			&& bgap >= IntronPrm.llmt) {
			stripe((Seq**) seqs, wdw, alprm.sh); // intron ?
			scr += pincerTrcbkS_ng(cmode);
...
//			scr += pincerTrcbkS_ng(seqs, alnv, cmode);
	...
	if (cmode == 2 && ovr == 0) {
	    ...
	    scr -= creepback(ovr, slmt, bab);
	    scr -= creepfwrd(ovr, slmt, bab);
	    if (cmode == 2 && (qck || ovr < IntronPrm.tlmt
		|| dpspace >= alprm.maxsp)) {
		if (wjxt) {
		    int	br = wjxt->jy - term;
		...
		scr += lspS_ng();
//	    } else {
//		scr += pincerTrcbkH_ng(&wdw, alnv, cmode);
	    }
	}
	rest_range((Seq**) seqs, rng, 2);
	a->inex.exgl = ainex.exgl;
	b->inex.exgl = binex.exgl;
	if (--level == algmode.lvl && jxt) {
	    wjxt->jx = a->len;
	    wjxt->jy = b->len;
	}
	delete wl;
...
SKL* Aln2s1::globalS_ng(VTYPE* scr)
{
	...
	if (algmode.qck) {
	    ...
	    *scr = seededS_ng(algmode.lvl, 1, bab);
	} else
	    *scr = lspS_ng();
	wsk.n = (int) mfd->size();
	...
	return (trimskl((Seq**) seqs, stdskl(&skl)));
}


VTYPE HomScoreS_ng(mSeq* seqs[], PwdM* pwd)
{
	WINDOW	wdw;
...
Colonies* swg1stS_ng(mSeq* seqs[], PwdM* pwd, VTYPE* scr)
{
	WINDOW	wdw;
...
SKL* swg2ndS_ng(mSeq* seqs[], PwdM* pwd, VTYPE* scr, COLONY* clny)
{
...
SKL* alignS_ng(mSeq* seqs[], PwdM* pwd, VTYPE* scr)
{
	WINDOW	wdw;

	if (!algmode.blk && algmode.mns != 1) {
	    if (algmode.mns == 2) antiseq((Seq**) seqs + 1);
