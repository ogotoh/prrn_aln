...
static	const VTYPE  NEVSELP = (VTYPE) (INT_MAX / 8 * 7);
...
void AlnIdn::output(SKL* slctd, VTYPE scr)
{
	Gsinfo	gsi;
	gsi.skl = slctd;
	gsi.scr = skl_rngB_ng((const Seq**) seqs, &gsi, pwd);
	GAPS*	gaps[2] = {0, 0};
	skl2gaps(gaps, slctd);
	toimage(gaps, 2);
	FSTAT&	fst = gsi.fstat;
...
int idn_main(CalcServer<Seq>* svr, Seq* seqs[], ThQueue<Seq>* q = 0)
{
	...
	if (algmode.nsa) {	// alignment
		...
		if (algmode.lcl) skl = trimskl((const Seq**) seqs, skl);
		m_thread_Lock(q);
...
void idn_setup(CalcServer<Seq>* svr)
{
	...
	PwdB*	pwd = new PwdB((const Seq**) svr->in_face);
	svr->prm = (void*) pwd;
}
