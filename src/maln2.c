...
struct	ConvPrm {int maxmem, sndmem,  minmem; Simmtx* simmtx;};
...
bool PwdM::advised_sim2(mSeq* seqs[])
{
	...
	aprof = nt >= thr_gfq_22 || nj == 1;
	bprof = nj > b->code->max_code;
	return (true);
}

static int convert_main(CalcServer<mSeq>* svr, mSeq* sqs[], ThQueue<mSeq>* q)
{
	ConvPrm*	maxin = (ConvPrm*) svr->prm;
	mSeq*&	sd = sqs[0];
	if (sd->many > 1) {
	    sd->setsimmtx(maxin->simmtx);
	    sd->convseq(VECTOR);
	    sd->convseq(VECPRO);
	}
	return (OK);
...
{
	ConvPrm	maxin = {0, 0, INT_MAX, sm};
	for (int i = 0; i < nos; ++i) {
	    int&	nn = seqs[i]->many;
	    if (nn > maxin.sndmem) {
		if (nn > maxin.maxmem) maxin.maxmem = nn;
		else	maxin.sndmem = nn;
	    }
	    if (nn < maxin.minmem) maxin.minmem = nn;
	}
	int	nt = 2 * maxin.sndmem + maxin.maxmem;
	if (nt < thr_gfq_21) return;
	CalcServer<mSeq> svr1(IM_SNGL, &maxin, &convert_main, 0, 0, seqs, nos);
	svr1.autocomp();
}
...
VTYPE selfAlnScr(mSeq* sd, Simmtx* sm)
{
	if (!sm) sm = getSimmtx(0);
	mSeq*	sqs[2] = {sd, sd};
	return calcscore_grp(sqs, 0);
}

Msap::Msap(mSeq* sd, int mode, ALPRM* ap) 
...
PwdM::PwdM(mSeq** seqs, ALPRM* alp, bool prf) 
	: PwdB((Seq**) seqs, alp), a(seqs[0]), b(seqs[1]), prof(prf), 
...
VTYPE Msap::selfi(const mSeqItr& asi)
{
	VTYPE 	s = 0;
...
VTYPE Msap::crg1i(const int* gla, const mSeqItr& asi)
{
...
/*VTYPE PwdM::sim00(const mSeqItr& asi, const mSeqItr& bsi)
{
	return (0);
}
*/

VTYPE PwdM::sim11(const mSeqItr& asi, const mSeqItr& bsi)
{
	return (simmtx->mtx[*asi.res][*bsi.res]);
}

VTYPE PwdM::sim12i(const mSeqItr& asi, const mSeqItr& bsi)
{
...
VTYPE PwdM::sim13(const mSeqItr& asi, const mSeqItr& bsi)
{
	return (bsi.vss[bsi.felm + *asi.res]);
}

VTYPE PwdM::sim21i(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
...
VTYPE PwdM::sim22i(const mSeqItr& asi, const mSeqItr& bsi)
{
	VTYPE 	s = 0;
const 	CHAR*	ca = asi.res;
const 	CHAR*	cb = bsi.res;
...
VTYPE PwdM::sim23i(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	ta = ca + asi.many;
...
VTYPE PwdM::sim31(const mSeqItr& asi, const mSeqItr& bsi)
{
	return (asi.vss[asi.felm + *bsi.res]);
}

VTYPE PwdM::sim32i(const mSeqItr& asi, const mSeqItr& bsi)
{
...
VTYPE PwdM::sim33(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	VTYPE*	va = asi.vss + asi.felm;
...
VTYPE PwdM::sim33_n(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	VTYPE*	va = asi.vss + asi.felm;
const 	VTYPE*	vb = bsi.vss;
	VTYPE	s = 0;
...
void PwdM::stt11(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	cb = bsi.res;
	if (NotGap(ca)) {
...
void PwdM::stt12i(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	cb = bsi.res;
const 	CHAR*	ct = cb + bsi.many;
...
void PwdM::stt13(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	VTYPE*	vb = bsi.vss;
	if (NotGap(ca)) {
	    if (vb) {
		CHAR caa = *ca;
		if (DvsP == TxP && caa == SER2) caa = SER;
		stt->mch += vb[caa];
		stt->mmc += vbn - vb[gap_code] - vb[caa] - vb[nil_code];
		stt->unp += vb[gap_code];
	    } else
		stt->unp += bsi.dns->efq;
...
void PwdM::stt21i(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	ct = ca + asi.many;
const 	CHAR*	cb = bsi.res;
...
void PwdM::stt22i(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	cb = bsi.res;
const 	CHAR*	ta = ca + asi.many;
...
void PwdM::stt23i(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	VTYPE*	vb = bsi.vss;
	...
	if (ca) {
	  ...
	  for ( ; ca < ta; ++ca) {
	    if (IsntGap(*ca)) {
	      if (vb) {
		stt->mch += vb[*ca];
		stt->mmc += vbn - vb[gap_code] - vb[*ca] - vb[nil_code];
		stt->unp += vb[gap_code];
...
void PwdM::stt31(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
...
const 	CHAR*	cb = bsi.res;
	if (NotGap(cb)) {
	    if (va) {
		CHAR	cbb = *cb;
		if (DvsP == PxT && cbb == SER2) cbb = SER;
		stt->mch += va[cbb];
		stt->mmc += van - va[gap_code] - va[cbb] - va[nil_code];
		stt->unp += va[gap_code];
	    } else
		stt->unp += asi.dns->efq;
...
void PwdM::stt32i(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	VTYPE*	va = asi.vss;
const 	CHAR*	cb = bsi.res;
	...
	if (cb) {
	  ...
	  for ( ; cb < tb; ++cb) {
	    if (IsntGap(*cb)) {
	      if (va) {
		stt->mch += va[*cb];
		stt->mmc += van - va[gap_code] - va[*cb] - va[nil_code];
		stt->unp += va[gap_code];
	      } else if (IsTrueGap(*cb))
		stt->unp += asi.dns->efq;
...
void PwdM::stt33(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	VTYPE*	va = asi.vss;
const 	VTYPE*	vb = bsi.vss;
...
// penalty fo unpaired residues = gap extension penalty

VTYPE PwdM::unp1(const mSeqItr& asi, const mSeqItr& bsi)
{
...
VTYPE  PwdM::newgap2(const mSeqItr& asi, const CHAR* bs, const int* glb)
{
	VTYPE	g = 0;
const 	CHAR*	ts = bs + bn;
#if USE_WEIGHT
...
VTYPE PwdM::crg11(const int* gla, const int* glb, const mSeqItr& asi, const mSeqItr& bsi, int d3)
{
const 	CHAR*	as = asi.res;
...
VTYPE PwdM::crg12i(const int* gla, const int* glb, const mSeqItr& asi, const mSeqItr& bsi, int d3)
{
const 	CHAR*	as = asi.res;
const 	CHAR*	bs = bsi.res;
...
VTYPE PwdM::crg21i(const int* gla, const int* glb, const mSeqItr& asi, const mSeqItr& bsi, int d3)
{
const 	CHAR*	as = asi.res;
const 	CHAR*	bs = bsi.res;
...
VTYPE PwdM::crg22i(const int* gla, const int* glb, 
	const mSeqItr& asi, const mSeqItr& bsi, int d3)
{
const 	CHAR*	as = asi.res;
const 	CHAR*	bs = bsi.res;
...
VTYPE PwdM::frw11(const int* a_gla, const int* a_glb, const int* b_gla, const int* b_glb)
{
	return (((*a_gla > *a_glb && *b_gla <= *b_glb) ||
		 (*a_gla < *a_glb && *b_gla >= *b_glb))? -Basic_GOP: 0);
}

VTYPE PwdM::frw12i(const int* a_gla, const int* a_glb, const int* b_gla, const int* b_glb)
{
...
VTYPE PwdM::frw21i(const int* a_gla, const int* a_glb, const int* b_gla, const int* b_glb)
{
...
VTYPE PwdM::frw22i(const int* a_gla, const int* a_glb, const int* b_gla, const int* b_glb)
{
const 	int*	ta = a_gla + an;
const 	int*	tb = b_glb + bn;
	int	nn = 0;
...
VTYPE Msap::self_p(const mSeqItr& asi)
{
const 	CHAR*	ca = asi.res;
	VTYPE 	s = 0;
const 	CHAR*	cb = ca;
...
VTYPE PwdM::sim12i_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	cb = bsi.res;
const 	CHAR*	tb = cb + bsi.many;
...
VTYPE PwdM::sim13_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	VTYPE*	vb = bsi.vss + bsi.felm;
...
VTYPE PwdM::sim21i_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	ta = ca + asi.many;
...
VTYPE PwdM::sim22i_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	cb = bsi.res;
const 	CHAR*	ta = ca + asi.many;
...
VTYPE PwdM::sim23i_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	ta = ca + asi.many;
const 	VTYPE*	vb = bsi.vss + bsi.felm;
...
VTYPE PwdM::sim31_p(const mSeqItr& asi, const mSeqItr& bsi)
{
...
VTYPE PwdM::sim32i_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	VTYPE*	va = asi.vss + asi.felm;
const 	CHAR*	cb = bsi.res;
const 	CHAR*	tb = cb + bsi.many;
...
VTYPE PwdM::sim33_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	VTYPE*	va = asi.vss;
const 	VTYPE*	vb = bsi.vss;
const 	VTYPE*	tb = vb + bsi.felm;
...
VTYPE PwdM::sim33_np(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	VTYPE*	va = asi.vss;
const 	VTYPE*	vb = bsi.vss;
	VTYPE	s = ab_u0 * (va[asi.eth_code] + vb[bsi.eth_code]);
...
VTYPE PwdM::unpa_p(const mSeqItr& asi, const mSeqItr& bsi)
{
	return (asi.dns->cfq * bsi.dns->efq * -alnprm.u +
		asi.dns->dfq * a_u0 + bsi.dns->efq * b_u0);
}

VTYPE PwdM::unpb_p(const mSeqItr& bsi, const mSeqItr& asi)
{
...
VTYPE Msap::selfw(const mSeqItr& asi)
{
	VTYPE 	s = 0;
const 	CHAR*	ca = asi.res;
const 	CHAR*	cb = ca;
...
VTYPE PwdM::sim12w(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	cb = bsi.res;
const 	CHAR*	ct = cb + bsi.many;
	VTYPE	s = 0;
...
VTYPE PwdM::sim21w(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	ct = ca + asi.many;
	VTYPE	s = 0;
...
VTYPE PwdM::sim22w(const mSeqItr& asi, const mSeqItr& bsi)
{
	VTYPE 	s = 0;
const 	CHAR*	ca = asi.res;
const 	CHAR*	cb = bsi.res;
...
VTYPE PwdM::sim23w(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	ta = ca + asi.many;
const 	VTYPE*	vb = bsi.vss + bsi.felm;
...
VTYPE PwdM::sim32w(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	VTYPE*	va = asi.vss + asi.felm;
const 	CHAR*	cb = bsi.res;
const 	CHAR*	tb = cb + bsi.many;
...
void PwdM::stt12w(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	cb = bsi.res;
const 	CHAR*	ct = cb + bsi.many;
...
void PwdM::stt21w(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	ct = ca + asi.many;
const 	CHAR*	cb = bsi.res;
...
void PwdM::stt22w(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	cb = bsi.res;
const 	CHAR*	ta = ca + asi.many;
...
void PwdM::stt23w(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	VTYPE*	vb = bsi.vss;
	...
	if (ca) {
	  ...
	  for (FTYPE* wa = wta; ca < ta; ++ca, ++wa) {
	    if (IsntGap(*ca)) {
	      if (vb) {
		stt->mch += vb[*ca] * *wa;
		stt->mmc += (vbn - vb[gap_code] - vb[*ca] - vb[nil_code]) * *wa;
...
void PwdM::stt32w(FSTAT* stt, const mSeqItr& asi, const mSeqItr& bsi)
{
const 	VTYPE*	va = asi.vss;
const 	CHAR*	cb = bsi.res;
	...
	if (cb) {
	  ...
	  for ( ; cb < tb; ++cb, ++wb) {
	    if (IsntGap(*cb)) {
	      if (va) {
		stt->mch += va[*cb] * *wb;
		stt->mmc += (van - va[gap_code] - va[*cb] - va[nil_code]) * *wb;
...
VTYPE Msap::crg1w(const int* gla, const mSeqItr& asi)
{
const	CHAR*	as = asi.res;
const 	FTYPE*	pw = pairwt;
	VTYPE	g = 0;
...
VTYPE PwdM::frw12w(const int* a_gla, const int* a_glb, const int* b_gla, const int* b_glb)
{
const 	int*	tb = b_glb + bn;
	VTYPE	t = 0;
...
VTYPE PwdM::frw21w(const int* a_gla, const int* a_glb, const int* b_gla, const int* b_glb)
{
const 	int*	ta = a_gla + an;
	VTYPE	t = 0;
...
VTYPE PwdM::frw22w(const int* a_gla, const int* a_glb, const int* b_gla, const int* b_glb)
{
const 	int*	ta = a_gla + an;
const 	int*	tb = b_glb + bn;
	VTYPE	g = 0;
...
VTYPE Msap::selfw_p(const mSeqItr& asi)
{
const 	CHAR*	ca = asi.res;
	VTYPE 	s = 0;
const 	CHAR*	cb = ca;
...
VTYPE PwdM::sim11w_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	cb = bsi.res;
...
VTYPE PwdM::sim12w_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	cb = bsi.res;
const 	CHAR*	tb = cb + bsi.many;
...
VTYPE PwdM::sim21w_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	ta = ca + asi.many;
...
VTYPE PwdM::sim22w_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	cb = bsi.res;
const 	CHAR*	ta = ca + asi.many;
...
VTYPE PwdM::sim23w_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	ca = asi.res;
const 	CHAR*	ta = ca + asi.many;
...
VTYPE PwdM::sim32w_p(const mSeqItr& asi, const mSeqItr& bsi)
{
const 	CHAR*	cb = bsi.res;
const 	CHAR*	tb = cb + bsi.many;
	VTYPE	s = 0;
...
VTYPE	PwdM::sim2_sshp(const mSeqItr& asi, const mSeqItr& bsi)
{
	VTYPE	scr = 0;
...
VTYPE HomScore(mSeq* seqs[], PwdM* pwdm, long rr[])
{
	...
	switch (pwdm->alnmode) {
	    ...
	    case NGP_ALH: scr = HomScoreH<DPUH>(seqs, pwdm); break;
	    case HLF_ALH:
	    case RHF_ALH: scr = HomScoreH<DPUH_hf>(seqs, pwdm); break;
	    case NGP_ALS: scr = HomScoreS_ng(seqs, pwdm); break;
	    default:
...
retry:
	if (a->left == a->right || b->left == b->right) {
	  ...
	  switch (pwdm->alnmode) {
	    ...
	    case NGP_ALH: case HLF_ALH: case RHF_ALH: case NGP_ALS:
		skl = alignH<DPUH>(seqs, pwdm, scr); break;
	    default: 
		fatal("Mode %d is not supported !\n", pwdm->alnmode);
	  }
	} else {
	  switch (pwdm->alnmode) {
	    ...
	    case NGP_ALH: skl = alignH<DPUH>(seqs, pwdm, scr); break;
	    case HLF_ALH:
	    case RHF_ALH: skl = alignH<DPUH_hf>(seqs, pwdm, scr); break;
	    case NGP_ALS: skl = alignS_ng(seqs, pwdm, scr); break;
	    default: 
		fatal("Mode %d is not supported !\n", pwdm->alnmode);
	  }
	}
	...
	if (!skl || !skl->n || !GsI) {
		...
		if (skl->m) {
		  switch (pwdm->alnmode) {
//		    case NGP_ALH: skl_rngH_ng(seqs, GsI, pwdm); break;
		    case NGP_ALH: skl_rngH<DPUH>(seqs, GsI, pwdm); break;
		    case RHF_ALH:
//		    case HLF_ALH: skl_rngH_hf(seqs, GsI, pwdm); break;
		    case HLF_ALH: skl_rngH<DPUH_hf>(seqs, GsI, pwdm); break;
		    case NGP_ALS: skl_rngS_ng(seqs, GsI, pwdm); break;
		    default:
		  ...
		  GsI->scr -= pwdm->GapPenalty(Ip_equ_k) * GsI->noeij;
		}
	    } else {
		GsI->skl = stdskl(&skl);
		int	num = skl->n;
		...
		pss.calcSpScore(GsI);
		if (OutPrm.trimend) skl = trimskl((Seq**) seqs, skl);
		GsI->skl = 0;
	    }
	}
	return (skl);
...
mSeq* align2(mSeq** sqs, mSeq* msd, ALPRM* alp)
{
	...
	if (algmode.lcl) {
	  int	tl = 0;
	  int	tr = 0;
	  int	ad = skl[2].m - skl[1].m;
	  int	bd = skl[2].n - skl[1].n;
	  int	ltgap = ad - bd;	// terminal gap
	  if (skl[1].m || skl[1].n) {	// internal range
	    ++tl;			// space for interpolation
	    if (!ltgap && skl[1].m != skl[1].n) ++tl;
	  } else if (!ad || !bd)	// left terminal gap
	    deltaleft = max(ad, bd);
	  SKL*	boo = skl + num;
	  int	at = a->len - boo->m;
	  int	bt = b->len - boo->n;
	  ...
	  ad = boo[1].m - boo->m;
	  bd = boo[1].n - boo->n;
	  int	rtgap = ad - bd;	// terminal gap
	  if (at || bt) {		// internal range
	    ++tr;			// space for interpolation
	    if (!rtgap && at != bt) ++tr;
	  } else if (!ad || !bd)
	    deltaright = max(ad, bd);
	  num += tl + tr;
	  if (num > skl->n) {
	    nsk = new SKL[num + 2];
	    nsk->n = num; nsk->m = skl->m;
	    vcopy(nsk + tl + 1, skl + 1, skl->n);
	    if (tl) {
		nsk[1].m = nsk[1].n = 0;
		boo = nsk + 3;
		if (ltgap || skl[1].m != skl[1].n) {	// interpolate
		    int	mn = min(boo->m, boo->n);
		    nsk[2].m = boo->m - mn;
		    nsk[2].n = boo->n - mn;
		}
		deltaleft = max(boo->m, boo->n);
	    }
	    if (tr) {
		nsk[num].m = a->len;
		nsk[num].n = b->len;
		if (rtgap || at != bt) {	// interpolate
		    boo = nsk + num - 2;
		    at = a->len - boo->m;
		    bt = b->len - boo->n;
		    int	mn = min(at, bt);
		    nsk[num - 1].m = boo->m + mn;
		    nsk[num - 1].n = boo->n + mn;
		}
		deltaright = max(at, bt);
	    }
	    nsk[num + 1].m = nsk[num + 1].n = EOS;
	  }
	}
	msd = syntheseq(msd, sqs, nsk);
	msd->left = deltaleft;
	msd->right = msd->len - deltaright;
	if (nsk != skl) delete[] nsk;
	delete[] skl;
	gswap(msd->sigII, alninf.sigII);
	if (pwd.swp) gswap(sqs[0], sqs[1]);
	return (msd);
}
...
FTYPE alnscore2dist(mSeq* sqs[], PwdM* pwd, int* ends)
{
	mSeq*&	a = sqs[0];
	mSeq*&	b = sqs[1];
	VTYPE	scr;
	...
	if (algmode.lcl) trimskl((Seq**) sqs, stdskl(&skl));
	int	al = skl[1].m;
	int	bl = skl[1].n;
	int	ar = skl[skl->n].m;
	int	br = skl[skl->n].n;
	...
	FTYPE	denome = (selfAlnScr(a, pwd->simmtx) / (a->sumwt * a->sumwt)
		+ selfAlnScr(b, pwd->simmtx) / (b->sumwt * b->sumwt)) / 2;
...
Wilip::Wilip(mSeq* seqs[], PwdB* pwd, INT level)
{
	Seq*	temp[2];
	temp[0] = seqs[0]->many > 1? seqs[0]->consenseq(): seqs[0];
	temp[1] = seqs[1];
	Wlp	wln(temp, pwd, level);
