...
struct	FstatSvrPrm {
	...
	FTYPE*	mfct;
	FTYPE*	dist;
	FstatSvrPrm(FTYPE* f, FTYPE* d) 
	    : pwd(0), mfct(f), dist(d)  {}
	~FstatSvrPrm() {delete pwd;}
};
...
struct ScrSvrPrm {
	...
	FTYPE*	scr1;
	FTYPE*	dist;
	ScrSvrPrm(FTYPE* s1, FTYPE* d)
	    : pwd(0), scr1(s1), dist(d) {}
	~ScrSvrPrm() {delete pwd;}
};
...
static	void*	dvx_main(void* arg);
static	FTYPE*	calcdistseq(Seq* sd, Subset* ss);
...
static	void	dpaln_setup(CalcServer<mSeq>* svr)
{
	prePwd((const Seq**) svr->in_face);
	FstatSvrPrm*	svp = (FstatSvrPrm*) svr->prm;
	svp->pwd = new PwdM(svr->in_face);
}

static int selfmatch(CalcServer<mSeq>* svr, mSeq* sqs[], ThQueue<mSeq>* q)
{
	FstatSvrPrm*	svp = (FstatSvrPrm*) svr->prm;
	...
	mSeq*	twin[2] = {sd, sd};
	PwdM*	pwd = dpaln_reset(svp->pwd, twin, q);
	Gsinfo	gsi(nogap_skl(sd));
	PreSpScore	pss(twin, pwd);
	pss.calcSpScore(&gsi);
	FTYPE	denm = gsi.fstat.mch + gsi.fstat.mmc + gsi.fstat.gap;
	if (!algmode.lcl) denm += gsi.fstat.unp;
	svp->mfct[sd->sid] = gsi.fstat.mch / denm;
	if (pwd != svp->pwd) delete pwd;
	return (OK);
}

static FTYPE fstat2dist(mSeq** sqs, FSTAT& cst, FTYPE* mfct)
{
	mSeq*&	a = sqs[0];
	mSeq*&	b = sqs[1];

	FTYPE	cr = cst.mch;
	FTYPE	denm = cst.mch + cst.mmc + cst.gap;
	if (!algmode.lcl) denm += cst.unp;
	denm *= sqrt(mfct[a->sid] * mfct[b->sid]);
	cr /= denm;
	if (cr > 1) cr = 1;
	return ((FTYPE)	pamcorrect((1 - cr), a->inex.molc));
}
...
static int dpaln_main(CalcServer<mSeq>* svr, mSeq* sqs[], ThQueue<mSeq>* q)
{
	...
	mSeq*&	a = sqs[0];
	mSeq*&	b = sqs[1];
	svp->dist[elem(a->sid, b->sid)] = 
	    fstat2dist(sqs, gsi.fstat, svp->mfct);
	delete[] skl;
	if (pwd != svp->pwd) delete pwd;
	return (OK);
}
...
static int dpscore(CalcServer<mSeq>* svr, mSeq* sqs[], ThQueue<mSeq>* q)
{
	ScrSvrPrm*	svp = (ScrSvrPrm*) svr->prm;
	...
	mSeq*&   a = sqs[0];
	mSeq*&   b = sqs[1];
	FTYPE	denome = sqrt(svp->scr1[a->sid] * svp->scr1[b->sid]);
	if (a->many == 1 && b->many == 1)
	    dst = alnscore2dist((Seq**) sqs, svp->pwd, ends, denome);
	else {
	    PwdM*	pwd = dpaln_reset(svp->pwd, sqs, q);
	    dst = alnscore2dist(sqs, pwd, ends, denome);
	    if (pwd != svp->pwd) delete pwd;
	}
	int	al = a->left;
	int	bl = b->left;
...
static int selfscr(CalcServer<mSeq>* svr, mSeq* sqs[], ThQueue<mSeq>* q)
{
	...
	mSeq*&	a = sqs[0];
	FTYPE	denom = (FTYPE) selfAlnScr(a) / (a->sumwt * a->sumwt);
	svp->scr1[a->sid] = denom;
	return (OK);
}

static void* dvx_main(void* arg)
{
	SvrPrm*	svp = (SvrPrm*) arg;

	for (int k = *svp->comb; k < svp->comb[1]; ++k) {
	    int	t[2];
	    decomb(t, k);
	    int&	i = t[0];
	    int&	j = t[1];
	    Subset*&	ss = svp->ss;
	    if (ss) svp->dist[k] = pairdvn(svp->sd, ss->group[i], ss->group[j]);
	    else    svp->dist[k] = pairdvn(svp->sd, i, j);
	}
	return (void*) 0;
}
...
FTYPE* calcdist(mSeq** sbuf, int nn, DistCal realign)
{
	convertseqs(sbuf, nn, getSimmtx(0));
	FTYPE*    dist = new FTYPE[ncomb(nn)];
	if (realign == DynAln) {
	    FTYPE*	mfct = new FTYPE[nn];
	    FstatSvrPrm	svp(mfct, dist);
	    CalcServer<mSeq> svr1(IM_SNGL, (void*) &svp, &selfmatch, 0, 0, sbuf, nn);
	    svr1.auto_comp();
	    CalcServer<mSeq> svr2(IM_EVRY, (void*) &svp, 
		&dpaln_main, &dpaln_setup, 0, sbuf, nn);
	    svr2.auto_comp();
	    delete[] mfct;
	} else if (realign == DynScr) {
	    FTYPE*	scr1 = new FTYPE[nn];
	    ScrSvrPrm	svp(scr1, dist);
	    CalcServer<mSeq> svr1(IM_SNGL, (void*) &svp, &selfscr, 0, 0, sbuf, nn);
	    svr1.auto_comp();
	    CalcServer<mSeq> svr2(IM_EVRY, (void*) &svp,
		&dpscore, &dpaln_setup, 0, sbuf, nn);
	    svr2.auto_comp();
	    delete[] scr1;
	} 
	return (dist);
...
static FTYPE* calcdistseq(Seq* sd, Subset* ss)
{
	...
	int	nc = ncomb(nn);
	FTYPE*	dist = new FTYPE[nc];
	if (thread_num <= 1 || nc <= thread_num) {
	    for (int j = 1, k = 0; j < nn; ++j) {
		for (int i = 0; i < j; ++i, ++k) {
		    if (ss) dist[k] = pairdvn(sd, ss->group[i], ss->group[j]);
		    else    dist[k] = pairdvn(sd, i, j);
		}
	    }
	} else {
	    int binsize = nc / thread_num;
	    int ne = nc / binsize;
	    int	na = nc - binsize * ne;
	    pthread_t* handle = new pthread_t[thread_num];
	    int*	comb = new int[thread_num + 1];
	    SvrPrm*	svp = new SvrPrm[thread_num + 1];
	    SvrPrm*	wvp = svp;
	    for (int i = 0, n = 0; i <= thread_num; ++i, ++wvp) {
		comb[i] = n;
		n += binsize;
		if (i < na) ++n;
		wvp->sd = sd;
		...
		pthread_create(&handle[i], NULL, dvx_main, 
		    (void*) &svp[i]);
	    }
	    for (int i = 0; i < thread_num; ++i) {
...
FTYPE* calcdist(Seq* sd, Subset* ss)
{
	...
	if (distPrm.realign == ThisAln) {
	    ...
	    dist = calcdist(sbuf, nn, distPrm.realign);
	    clearseq(sbuf, nn); 
	    delete[] sbuf;
	}
	return (dist);
	...
	: realign(realin), bias(0), scrrng(0)
{
	...
	numb = tv.num;
	if (realign == DynAln) {
	    FTYPE*	mfct = new FTYPE[numb];
	    dist = new FTYPE[ncomb(numb)];
	    FstatSvrPrm	svp(mfct, dist);
	    AlnServer<mSeq> svr1(argc, argv, IM_SNGL, catalog, (void*) &svp, &selfmatch);
	    svr1.auto_comp();
	    AlnServer<mSeq> svr2(argc, argv, IM_EVRY, catalog, (void*) &svp,
		&dpaln_main, &dpaln_setup);
	    svr2.auto_comp();
	    delete[] mfct;
	} else if (realign == DynScr) {
	    FTYPE*	scr1 = new FTYPE[numb];
	    dist = new FTYPE[ncomb(numb)];
	    ScrSvrPrm	svp(scr1, dist);
	    AlnServer<mSeq> svr1(argc, argv, IM_SNGL, catalog, (void*) &svp, &selfscr);
	    svr1.auto_comp();
	    AlnServer<mSeq> svr2(argc, argv, IM_EVRY, catalog, (void*) &svp,
		&dpscore, &dpaln_setup);
	    svr2.auto_comp();
