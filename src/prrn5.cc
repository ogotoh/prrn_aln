...
static	mSeq*	makemsa(mSeq** seqs, int nn);
static	mSeq*	makemsa(mSeq** seqs, int nn, ALPRM* alp, bool sglthrd);
static	mSeq*	makemsa(int argc, const char** argv, InputSeqTest* tv);
...
static	const	int	too_short_for_kmer = 100;
...
static	const	char*	cds_coordinate = 0;
...
void usage()
{
	fputs("*** prrn version 5.2.0 <200923> ***\n\n", stderr);
...
void Prrn::gather(mSeq* sd, int* gr, GapsList* gbuf)
{
	if (gr) {
#if USE_WEIGHT
	    if (gbuf->num == 1) {
...
SKL* Prrn::divideseq(MCTYPE mcran, int* lst[], GapsList* gsubs[], VTYPE* pwt)
{
	...
	*pwt = calcfact(wlst, srl->ktree->lead + mcran);
	...
	glst->copy_to(gsubs[0], lst[0]);
	GAPS*	gp0 = gsubs[0]->delcommongap();
	glst->copy_to(gsubs[1], lst[1]);
	GAPS*	gp1 = gsubs[1]->delcommongap();
	if (gaps_free(gp0) && gaps_free(gp1))
	    return (0);		// no common gap
	gather(seqs[1], lst[0], gsubs[0]);
	gather(seqs[2], lst[1], gsubs[1]);
	SKL*	skl = gap2skl(gp0, gp1);
	return (skl);
}
	...
	VTYPE* pwt, MCTYPE* bch)
{
	...
	return (divideseq(mcran, lst, gsubs, pwt));
}
	...
	slst(0), sbuf(0), wlst(0), wbuf(0)
{
	...
	maxgaps = max(msd->len - maxgaps + 3, MaxNoGaps);
	...
	gather(seqs[2]);	// result is stored in seqs[2]
...
static mSeq* makemsa(mSeq** seqs, int num)
{
	int	many = 0;
	int	len = 0;
	for (int i = 0; i < num; ++i) {
	    many += seqs[i]->many;
	    if (seqs[i]->len > len) len = seqs[i]->len;
	}
	mSeq*	msd = new mSeq(many, len);
	seqs[0]->copyattr(msd);
	CHAR*	pss = msd->at(0);
	CHAR*	tss = msd->at(len);
	for (int i = 0, k = 0; i < num; ++i) {
	    mSeq*&	wsq = seqs[i];
	    CHAR*	ps = pss + k;
	    CHAR*	ts = wsq->at(wsq->len);
	    for (CHAR* qs = wsq->at(0); qs < ts; ps += many, qs += wsq->many)
		memcpy(ps, qs, wsq->many);
	    for ( ; ps < tss; ps += many)
		memset(ps, gap_code, wsq->many);
	    for (int j = 0; j < wsq->many; ++j)
		msd->sname->push((*wsq->sname)[j]);
	    k += wsq->many;
	}
	msd->postseq(tss);
	msd->mkthick();
...
static mSeq* makemsa(mSeq** seqs, int nn, ALPRM* alp, bool sglthrd)
{
	...
	for (int i = 0; i < nn; ++i) {
	    seqs[i]->sid = i;
	    if (seqs[i]->len < minlen) minlen = seqs[i]->len;
	}
	if (seqs[0]->isprotein()) minlen *= 3;
	DistCal	realin = (distPrm.contbt_spj > 0 || minlen < too_short_for_kmer)? 
	    DynAln: (DistCal) algmode.any;
...
static mSeq* makemsa(int argc, const char** argv, InputSeqTest* tv)
{
	mSeq**	seqs = new mSeq*[tv->many];
	initseq(seqs, tv->many);
	AlnServer<mSeq> svr(argc, argv, IM_SNGL, catalog, (void*) seqs, &getseq_main);
	svr.autocomp(false);
	if (cds_coordinate) {
	    AddEijInfo	eij(cds_coordinate);
	    eij.mksigii((Seq**) seqs, tv->many);
	}
	mSeq*	msd = tv->dels?
	    makemsa(seqs, tv->many): makemsa(seqs, tv->many, &alprm, true);
	clearseq(seqs, tv->many);
	delete[] seqs;
	return (msd);
}
...
static int cmpcds(const ORF* a, const ORF* b)
{
	if (a->pos > b->pos - b->len &&
	    a->pos - a->len < b->pos) return (a->frm - b->frm);
	return (a->pos - a->len - b->pos + b->len);
}

AddEijInfo::AddEijInfo(const char* fn)
	: n_mem(0), n_eij(0), coordinate(fn)
{
	fd = fopen(coordinate, "r");
	if (!fd) fatal(not_found, coordinate);
	bool	wasd = false;
	bool	isd = false;
	...
	n_eij = n_eij / 2 - n_mem;
	rewind(fd);
	cdsmem = new StrHash<long>(n_mem);
	while (fgets(str, MAXL, fd)) {
	    if (*str == '>') {
		char*	ps = str + 1;
		cdsmem->assign(car(ps), ftell(fd));
	    }
	}
}

void AddEijInfo::mksigii(Seq* host)
{
	ORF*	cdsdt = new ORF[n_eij];
	ORF*	wdt = cdsdt;
	int	step = host->isprotein()? 3: 1;
	for (int i = 0; i < host->many; ++i) {
	    KVpair<INT, long>*	kv = cdsmem->find((*host->sname)[i]);
	    if (!kv) continue;
	    fseek(fd, kv->val, SEEK_SET);
	    if (!fgets(str, MAXL, fd))
		fatal("Imperfect cds file: %s !\n", coordinate);
	    Seq	sd(1);
	    int	gr[2] = {i, -1};
	    host->extseq(&sd, gr, CPY_SEQ);
	    int	nseg = sd.getcds(fd, str, 3);
	    if (nseg == 0) continue;
	    if (sd.exons && ((sd.exons->left > sd.exons[sd.CdsNo - 1].left) ^ (nseg < 0)))
		vreverse(sd.exons, sd.CdsNo);
	    RANGE*	eij = sd.exons;
	    int	ress = 0;
	    int	cds = abs(eij->right - eij->left);
	    int	unp = 0;
	    int	gap = 0;
	    CHAR*	ss = sd.at(0);
	    CHAR*	ts = sd.at(sd.len);
	    for (CHAR* ps = ss; ps < ts; ++ps) {
		if (!IsntGap(*ps)) {
		    unp += step;
		    gap += step;
		    continue;
		...
		if (cds <= ress) {
		    ...
		    (wdt++)->len = unp;
		    ++eij;
		    cds += abs((eij->right - eij->left));
		}
		unp = 0;
		ress += step;
	    }
	}
	qsort((UPTR) cdsdt, (INT) n_eij, sizeof(ORF), (CMPF) cmpcds);
	int	n = 0;
	for (int i = 0; i < n_eij; ++n) {
	    RANGE	rng = {cdsdt[i].pos - cdsdt[i].len, cdsdt[i].pos};
	    int	j = i;
	    int	k = i + 1;
	    int	ingap = cdsdt[i].len;
	    for ( ; cdsdt[k].pos - cdsdt[k].len <= rng.right &&
		rng.left <= cdsdt[k].pos; ++k) {
		if (cdsdt[k].len < cdsdt[j].len) j = k;
		if (cdsdt[k].len > ingap) ingap = cdsdt[k].len;
	    }
	    if (ingap)
		for ( ; i < k; ++i) cdsdt[i].pos = cdsdt[j].pos;
	    else	i = k;
	}
	host->sigII = new SigII(n, n_eij, step);
	PFQ*	wfq = host->sigII->pfq;
	int*	wst = host->sigII->lst;
	for (int i = 0; i < n_eij; ) {
	    wfq->pos = cdsdt[i].pos;
	    while (cdsdt[i].pos == wfq->pos) {
		++wfq->num;
#if USE_WEIGHT
		if (host->weight) wfq->dns += host->weight[cdsdt[i].frm];
#endif
		*wst++ = cdsdt[i].frm;
		++i;
	    }
#if USE_WEIGHT
	    if (!host->weight) wfq->dns = wfq->num;
#endif
	    ++wfq;
	}
	*wfq = pfqend;
...
void AddEijInfo::mksigii(Seq** seqs, int mm)
{
	int	step = seqs[0]->isprotein()? 3: 1;
	for (int m = 0; m < mm; ++m) {
	    Seq*&	sd = seqs[m];
	    KVpair<INT, long>*  kv = cdsmem->find((*sd->sname)[0]);
	    if (!kv) continue;
	    fseek(fd, kv->val, SEEK_SET);
	    if (!fgets(str, MAXL, fd))
		fatal("Imperfect cds file: %s !\n", coordinate);
	    int	nseg = sd->getcds(fd, str, 3);
	    if (!nseg || sd->sigII) continue;
	    if (((sd->exons->left > sd->exons[sd->CdsNo - 1].left) ^ (nseg < 0)))
		vreverse(sd->exons, sd->CdsNo);
	    sd->sigII = new SigII(sd->CdsNo - 1, 0, step);
	    PFQ*	wfq = sd->sigII->pfq;
	    RANGE*	cr = sd->exons;
	    int 	cds = 0;
	    for (int i = 0; i < sd->CdsNo; ++i, ++wfq, ++cr) {
		wfq->num = (i < sd->CdsNo - 1)? 1: 0;
		wfq->pos = (cds += abs(cr->right - cr->left));
		wfq->gps = cr->right;
#if USE_WEIGHT
		wfq->dns = wfq->num;
#endif
	    }
	    delete[] sd->exons; sd->exons = 0; sd->CdsNo = 0;
	}
}
...
void Msa::phylsort()
{
	if (!srl) srl = new Ssrel(msd);
	lst_odr = new int[msd->many + 1];
	lst_idx = 0;
	lstodr(srl->ktree->root);
	lst_odr[lst_idx] = -1;
...
{
	int	nn = ncomb(srl->ss->num);
#if USE_WEIGHT
	if (srl->pairwt) {
	    if (srl->ss->num > thr_gfq_23)
		return (srl->ktree->sum_of_pairwt());
	    FTYPE   f = 0;
	    for (int i = 0; i < nn; ++i)
		f += srl->pairwt[i];
	    return (f);
...
mSeq* Msa::output()
{
	...
	if (algmode.nsa & 4 && span > 0) {
	    if (!srl) srl = new Ssrel(msd);
	    if (!srl->pairwt) srl->calcpw(msd);
#if USE_WEIGHT
	    FTYPE*   nopwt = 0;
	    std::swap(nopwt, srl->pairwt);
	    VTYPE   spscr = srl->pairsum_ss(msd, false);
	    std::swap(nopwt, srl->pairwt);
	    VTYPE   wspscr = srl->pairsum_ss(msd);
#else
	    VTYPE   spscr = srl->pairsum_ss(msd, false);
	    VTYPE   wspscr = spscr;
#endif	// USE_WEIGHT
