...
Kcomp::Kcomp(Seq* sd, INT sz, INT* ct, int i, int m) 
...
static int qdiv_main(CalcServer<Seq>* svr, Seq* seqs[], ThQueue<Seq>* q = 0)
{
	...
	char	str[MAXL];
	char*	kname = (sd->many > 1 && sd->spath)? 
		partfnam(str, (*sd->spath)[0], "be"): (*sd->sname)[0];
	m_thread_Lock(q);
	kcp->sname->push(kname);
	m_thread_Unlock(q);
...
int	main(int argc, const char** argv)
{
	...
	rv = ksvr.autocomp(true);
	ksvr.change_job(&kcmp_out);
	rv = ksvr.autocomp(false);
	eraStrPhrases();
	return (0);
}
