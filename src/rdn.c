	...
	: num(0), which(0)
{
	...
	Dhash<int, int>	numsubmems(sd->many);
...
static	const char mssg[] = 
	...
	if (dstsq->many == 1) {
	    if (dstsq->spath) dstsq->spath->assign((*dstsq->sname)[0]);
	    else	dstsq->spath = new Strlist((*dstsq->sname)[0]);
	} else {
	    sprintf(ans, "%s%d", src->sqname(), dstsq->many);
	    if (dstsq->spath) dstsq->spath->assign(ans);
	    else	dstsq->spath = new Strlist(ans);
#if REPORT_STAT
	    statseq(&matches, dstsq);
#endif
	}
	prntout(dstsq);
	*ans = '\0';
