...
void sltree_getoption(int& argc, const char**& argv)
{
	while (--argc > 0 && **++argv == OPTCHAR) {
	  ...
	  switch (argv[0][1]) {
		...
		algmode.any = (int) atoi(pn); break;
	    case 'D': show_dist= true; break;
	    case 'H':
	      if ((pn = getarg(argc, argv, true)))
		alprm.thr = (float) atof(pn); break;
	    case 'M': 
	      if ((pn = getarg(argc, argv, true)))
		OutPrm.MaxOut = atoi(pn); break;
	    case 'N': 
	      if ((pn = getarg(argc, argv, true)))
		min_memb = atoi(pn); break;
	    case 'X': 
	      if ((pn = getarg(argc, argv, true)))
		max_memb = atoi(pn); break;
	    case 'K': 
	      ...
	      break;
            case 'p':
         	switch (*pn) {
                  ...
                  case 'J': case 'm': case 'u': case 'v': setexprm_z(pn); break;
		  default: break;
		}
		break;
	    case 's':
...
void usage()
{
	fputs("Usage:\n\tsltree [-Hn -Nn -Xn -D] distance file !\n", stderr);
