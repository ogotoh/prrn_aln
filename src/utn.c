...
static	const	int ctab[] = {T, C, A, G};	/* U, C, A, G */
	...
	for (int i = 0; i < 4; i++) {
		...
		for (int k = 0; k < 4; k++) {
		    ...
		    fprintf(out_fd, "  %s  %3d", amino3[toaa(trip)], codontab[gc]);
		}
		putc('\n', out_fd);
	    }
	    putc('\n', out_fd);
...
Composit::Composit(Seq* sd)
	: codontab(0), TotalNseq(0), TotalNres(0), nsize2(0), 
	  molc(sd->inex.molc), sqcode(sd->code), each(get_def_input_mode() != IM_MULT)
{
	gccmp[0] = 0;
